...
export const createNewItemPopover = (board, eventFireLocation) => {
  ...
  for (let [itemType, itemTypeName] of Object.entries({
    note: "Note",
    todo: "TODO",
    list: "List",
    todolist: "TODO List",
...
export const listEntryPopover = (
  ...
  popoverHeader.textContent = !!inputEntry
    ? "Change the entry"
    : "Let's add new entry!";
...
export const boardPopover = (user, eventFireLocation, board = null) => {
  ...
  const popoverHeader = document.createElement("h4");
  popoverHeader.textContent = !!board
    ? "Renovate"
  ...
  popoverSubmit.addEventListener("click", (event) => {
    if (!!board) {
      ...
      const newBoard = user.createBoard(
        boardTitle.value,
        boardDescription.value,
        classifiedBoardTextures["small"][
          Math.floor(Math.random() * classifiedBoardTextures["small"].length)
        ],
      );
