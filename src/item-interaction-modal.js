...
export default function (board, item, itemDiv) {
  ...
  for (let fieldKey of Object.keys(itemStorageObject).slice(1)) {
    ...
    if (fieldKey === "dueDate") {
      ...
      entryHeader.textContent = itemStorageObject.entries.length
        ? "Current tasks"
        : "No tasks yet";
      ...
      for (let i = 0; i < entryObjects.length; i++) {
        ...
        const popoverEditEntry = listEntryPopover(
          item,
          itemDiv,
          entryObjects[i],
        );
    ...
    for (let field of itemDialog.getElementsByTagName("p")) {
      ...
      if (field.getAttribute("data-field-type") === "entries") {
        ...
        for (let entry of field.getElementsByClassName("entry-container")) {
          const entryContents =
            entry.getElementsByTagName("label")[0].textContent;
