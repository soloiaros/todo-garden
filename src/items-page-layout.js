...
const sortItems = (items, sortingOrderParam) => {
  ...
  const sortingOrder =
    sortingOrderParam || (sortingSelect ? sortingSelect.value : null);

  switch (sortingOrder) {
    case "by-date-created-incr":
      items.sort((item1, item2) =>
        compareDesc(item1.getDateCreated(), item2.getDateCreated()),
      );
      return items;
    case "by-date-created-dcr":
      items.sort((item1, item2) =>
        compareAsc(item1.getDateCreated(), item2.getDateCreated()),
      );
      return items;
    case "by-date-changed-incr":
      items.sort((item1, item2) =>
        compareDesc(item1.getDateChanged(), item2.getDateChanged()),
      );
      return items;
    case "by-date-changed-dcr":
      items.sort((item1, item2) =>
        compareAsc(item1.getDateChanged(), item2.getDateChanged()),
      );
      return items;
    case "by-name-incr":
      items.sort((item1, item2) => {
    ...
    for (let item of sortedItems) {
      ...
      for (let key in allItemFields) {
        if (!["type", "dateCreated", "dateChanged"].includes(key)) {
          ...
          if (key === "dueDate") {
            itemInfoField.innerText =
              "Deadline is on " + item.getDueDateReadable();
